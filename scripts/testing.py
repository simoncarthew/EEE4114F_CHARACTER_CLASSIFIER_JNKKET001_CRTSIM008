...
def save_trends(val_trend, train_trend, loss_trend, epochs, test_acc, no_layers, activation_type, learning_rate,dir_save_path = "results/trends/test_"):
    ...
    file_path += str(test_acc).replace('.', '_') + ".csv"
...
if __name__ == "__main__":
    ...
    for n_l in no_layers:
        for acc_type in activation_types:
            for l_r in learning_rates:
                ...
                save_trends(validation_trend,training_trend,loss_trend,epochs,acc,n_l,acc_type,l_r)
    ...
    # test best with dropout
    # models = [[F.leaky_relu,3,0.01],[F.leaky_relu,2,0.01],[F.elu,2,0.01]]
    # df = pd.DataFrame(columns=['activation_function', 'no_layers', 'layer_sizes', 'learning_rate', 'test_accuracy'])
    # for params in models:
    #     model = classifier.NumberClassifier(num_layers=params[1],layer_sizes=layer_sizes[:params[1]],activation=params[0],dropout_rate=0.2)
    #     validation_trend, training_trend, loss_trend, epochs = classifier.train_classifier(model,train_loader,val_loader,no_epochs,min_loss_chng,params[2])

    #     # print succesfule training
    #     print("Trained with " + params[0].__name__ + " with " + str(params[1]) + " layers at learning rate of " + str(params[2]))

    #     # validate model
    #     acc = classifier.validate_classifier(model,test_loader)

    #     # save trends to a csv
    #     save_trends(validation_trend,training_trend,loss_trend,epochs,acc,params[1],params[0],params[2],dir_save_path="results/drop_out_trends/dropout_")

    #     record = {
    #                 'activation_function': params[0].__name__,
    #                 'no_layers': params[1],
    #                 'layer_sizes': layer_sizes[:params[1]],
